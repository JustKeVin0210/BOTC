...
def get_ability_role(img_real_dir, ability_role_folder, save_folder):
    ...
    for img_ability_name in img_ability_name_list:
        ...
        img_real_alpha_resize = cv2.resize(img_real_alpha, (w_ability // 2, h_ability // 2), interpolation=cv2.INTER_NEAREST)

        img_ability[-h_ability // 2:, -w_ability // 2:, :3][img_real_alpha_resize > 127] = img_real_bgr_resize[img_real_alpha_resize > 127]
        img_ability[-h_ability // 2:, -w_ability // 2:, 3:] = np.maximum(img_ability[-h_ability // 2:, -w_ability // 2:, 3:],
...
def get_ability(img_origin_folder, save_path, role_path_list=role_path_list):
    for role_path in role_path_list:
        ...
        for ability_role_path in role_path["ability_role_path_list"]:
            ...
            get_ability_role(img_real_dir, ability_role_folder, save_folder)
